...
CREATE TABLE PERFORMER_TYPE ( PERFORMER_TYPE_ID INT GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1) PRIMARY KEY, TYPE_NAME VARCHAR2(255) NOT NULL );
CREATE TABLE PERFORMER_TYPE_A ( PERFORMER_TYPE_A_ID INT GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1) PRIMARY KEY, PERFORMER_TYPE_A_NAME VARCHAR2(255) NOT NULL );
CREATE TABLE PERFORMER_TYPE_B ( PERFORMER_TYPE_B_ID INT GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1) PRIMARY KEY, PERFORMER_TYPE_B_NAME VARCHAR2(255) NOT NULL );
...
CREATE TABLE LOCATIONS ( LOCATION_ID INT GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1) PRIMARY KEY, LOCATION_NAME VARCHAR(255) NOT NULL, COUNTRY_ID INT NOT NULL, CONSTRAINT COUNTRY_FK FOREIGN KEY (COUNTRY_ID) REFERENCES COUNTRIES (COUNTRY_ID) );
CREATE TABLE USERS( USER_ID INT GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1) PRIMARY KEY, USERNAME VARCHAR(255), EMAIL VARCHAR(255) CHECK (EMAIL LIKE '%@%'), PHONE_NUMBER INT CHECK (LENGTH(PHONE_NUMBER)<16), PASSWORD VARCHAR(255) CHECK (LENGTH(PASSWORD)>5), CITY_STATE_COUNTRY VARCHAR(255), NUM_OF_TICKETS_BOOKED INT );
CREATE TABLE BOOKINGS( TICKET_ID INT, EVENT_ID INT, USER_ID INT, TICKET_TYPE INT, PRIMARY KEY(USER_ID, TICKET_ID), CONSTRAINT FK_TICKET_EVENT_ID FOREIGN KEY (TICKET_ID, EVENT_ID) REFERENCES TICKETS(TICKET_ID, EVENT_ID), CONSTRAINT FK_USER_ID FOREIGN KEY(USER_ID) REFERENCES USERS(USER_ID) );
SELECT
  *
FROM
  PERFORMERS;
DROP TABLE EVENTS CASCADE CONSTRAINTS;
CREATE TABLE EVENTS ( EVENT_ID INT PRIMARY KEY, EVENT_NAME VARCHAR(255) NOT NULL, VENUE_ID INT NOT NULL, EVENT_DATE VARCHAR(255) NOT NULL CHECK (EVENT_DATE LIKE '__-__-____'), START_TIME VARCHAR(255) NOT NULL, END_TIME VARCHAR(255) NOT NULL, ORGANIZER_ID INT NOT NULL, PERFORMER_ID INT NOT NULL, EVENT_CATEGORY_ID INT NOT NULL, NUM_OF_TICKETS INT NOT NULL, NUM_OF_VIP_TICKETS INT NOT NULL, CONSTRAINT VENUE_FK FOREIGN KEY (VENUE_ID) REFERENCES VENUES (VENUE_ID), CONSTRAINT ORGANIZER_FK FOREIGN KEY (ORGANIZER_ID) REFERENCES ORGANIZERS (ORGANIZER_ID), CONSTRAINT PERFORMER_FK FOREIGN KEY (PERFORMER_ID) REFERENCES PERFORMERS (PERFORMER_ID), CONSTRAINT EVENT_CATEGORY_FK FOREIGN KEY (EVENT_CATEGORY_ID) REFERENCES EVENT_CATEGORY (EVENT_CATEGORY_ID) );
...
CREATE OR REPLACE TRIGGER EVENT_ID_CONTROL BEFORE INSERT ON EVENTS FOR EACH ROW BEGIN
  SELECT
...
CREATE OR REPLACE TRIGGER TICKET_CREATION_TRIGGER
...
INSERT INTO EVENTS (
    ...
    1,
    'Sample Event',
    1, -- Replace with a valid VENUE_ID
    '01-01-2023',
    '18:00',
    '22:00',
    1, -- Replace with a valid ORGANIZER_ID
    1, -- Replace with a valid PERFORMER_ID
    1, -- Replace with a valid EVENT_CATEGORY_ID
    6,
    5
);
...
set serveroutput on;
CREATE OR REPLACE TRIGGER TICKET_SEAT_NUM
...
BEGIN
  BEGIN
    -- Attempt to find an available seat
    SELECT SEAT_NUM
    INTO v_seat_num
    FROM SEATS
    ...
    -- If a seat is found, mark it as booked
    IF v_seat_num IS NOT NULL THEN
      UPDATE SEATS
      SET BOOKED = 'y'
      WHERE SEAT_NUM = v_seat_num;

      -- Assign the seat number to :NEW.SEAT_NUM
      :NEW.SEAT_NUM := v_seat_num;

      -- Output success message
      DBMS_OUTPUT.PUT_LINE('Seat assigned successfully: ' || v_seat_num);
    ELSE
      -- Output message for no available seat
      DBMS_OUTPUT.PUT_LINE('No available seat found for VENUE_ID ' || :NEW.VENUE_ID);
    END IF;

  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      -- Output message for exception
      DBMS_OUTPUT.PUT_LINE('No available seat found for VENUE_ID ' || :NEW.VENUE_ID);
  END;
END;
/
